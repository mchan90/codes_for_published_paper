...
#n_samples = [1, 5, 10, 20, 40, 60, 80, 100, 200, 400, 600, 800, 1000, 2000, 3000, 4000, 5000]
...
#    if (core==0):
#        print(n_sample*(n_level+1)*(n_time), max_time, error/tau_scale)
#    indx_n_sample += 1
#
#
#
...
#    comm.Bcast([samples_qcels,MPI.INT],root=0)
        ...
        for i_level in range(n_level+1):
            for i_time in range(n_time):
                for i_sample in range(n_sample):
                    ...
                    i_jobs += 1
    # allreduce
    amplitudes_tmp = np.zeros((n_iter,n_level+1,n_time,n_sample),dtype=complex)
    comm.Allreduce([amplitudes_sample_qcels, MPI.DOUBLE_COMPLEX], [amplitudes_tmp, MPI.DOUBLE_COMPLEX], op=MPI.SUM)
    amplitudes_sample_qcels = amplitudes_tmp
    # bcast amplitudes_sample_qcels
    #i_jobs = 0
    #for i_iter in range(n_iter):
    #    for i_level in range(n_level+1):
    ...
    #                amplitude = comm.bcast(amplitudes_sample_qcels[i_iter,i_level,i_time,i_sample],root=i_core)
    #                amplitudes_sample_qcels[i_iter,i_level,i_time,i_sample] = amplitude
    #                #comm.Bcast([amplitudes_sample_qcels[i_iter,i_level,i_time,i_sample],MPI.DOUBLE_COMPLEX],root=i_core)
    #                comm.Barrier()
    #                i_jobs  += 1
    # bcast end
    ...
    if (core==0):
        print(n_trotter, max_time, error/tau_scale)
    indx_n_trotter += 1
    end_time = datetime.now()
    elapsed = end_time-start_time
    elapsed = elapsed.total_seconds()
