...
beta_list = [0.1]
beta_list += [0.1+0.5 * j for j in range(1,9)]
#beta_list = [2.0]
...
# %%
if (core==0):
    with open('dE_vs_evolution_exact','w') as file_:
...
for beta in beta_list:
    error = 0.0
    if (error_for_beta_list[indx_beta]>1e-3):
        nmc = 16384
    if (error_for_beta_list[indx_beta]<1e-3):
        nmc = 1638400
    #max_time = 0.0
    for i_iter in range(n_iter):
        ...
        for alpha in range(1,n_hamiltonians):
            ...
            norm_core = np.zeros((1),dtype=float)
            dE1_core  = np.zeros((1),dtype=float)
            ...
            for ihd in range(nhd1):
                ...
                for imc in range(nmc):
                    ...
                    dE1_core += np.real(coeff * computed_value)
            ...
            comm.Allreduce(MPI.IN_PLACE, norm_core, op=MPI.SUM)
            comm.Allreduce(MPI.IN_PLACE, dE1_core, op=MPI.SUM)
            ...
            norm_core = np.zeros((1),dtype=float)
            dE1_core  = np.zeros((1),dtype=float)
            dE2_core  = np.zeros((1),dtype=float)
            ...
            for ihd in range(nhd2):
                ...
                for imc in range(nmc):
                    ...
                    dE2_core += np.real(coeff * computed_value)
            ...
            comm.Allreduce(MPI.IN_PLACE, norm_core, op=MPI.SUM)
            comm.Allreduce(MPI.IN_PLACE, dE1_core, op=MPI.SUM)
            comm.Allreduce(MPI.IN_PLACE, dE2_core, op=MPI.SUM)
    ...
    error /= n_iter
    if (core==0):
        print(beta, beta*C99, error)
    error_for_beta_list[indx_beta] = error
    indx_beta += 1
#

if (core==0):
    with open('dE_vs_evolution_sample','w') as file_:
        s = '# from exact gaussian, total number of samples for each beta = ' + str(n_pubs_sum*nmc)
        s += '\n'
        file_.write(s)
        indx_beta = 0
        for beta in beta_list:
