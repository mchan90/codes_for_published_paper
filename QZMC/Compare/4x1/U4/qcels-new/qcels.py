...
#time_max_list = np.arange(1800, 2002, 2)
#T0 = 8000
...
n_sample = 2048 # 1024 # int(15/eta**2*np.log(d))
    ...
    for i_iter in range(n_iter):
        for i_level in range(n_level+1):
            for i_time in range(n_time):
                #time_ = times_qcels[i_level,i_time] + d
...
# %%
if (core==0):
    with open('dE_vs_evolution_wo_sampling','w') as file_:
...
for time_max in time_max_list:
    ...
    if (core==0):
        #print(time_max, max_time, error)
        #print(d, max_time*tau_scale, error/tau_scale)
        print(time_max, max_evolution_time_list[indx_time], error/tau_scale)
    indx_time += 1
...
#n_sample = int(15/eta**2*np.log(d))
n_sample = 2048
max_n_trotter_list= [12, 30, 48, 64, 80, 100, 130, 164, 248, 330, 412, 500, 580, 664, 748, 830] # qzmc values
