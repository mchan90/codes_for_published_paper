...
#beta_list = np.linspace(0.1,4.0,num=41)
beta_list = np.arange(0.1, 4.1, 0.1)
#beta_list = [2.0]
norms_for_beta_list     = np.zeros(len(beta_list),dtype=float)
value_for_beta_list     = np.zeros(len(beta_list),dtype=float)
...
for beta in beta_list:
    ...
    value_for_beta_list[indx_beta] = eigen_energies_qzmc[-1]
...
for beta in beta_list:
    ...
    error_for_beta_list[indx_beta] = error
    value = comm.bcast(value_for_beta_list[indx_beta],root=i_core)
    value_for_beta_list[indx_beta] = value
    if (core==0):
...
#                    max_time = max(max_time,time_sum)
        ...
        for alpha in range(1,n_hamiltonians):
            ...
            #    print(st)
        #error += np.abs(eigen_energies_qzmc[-1] - eigen_energies_exact[0][-1,0])
        error += np.abs(eigen_energies_qzmc[-1] - value_for_beta_list[indx_beta])
    error /= n_iter
