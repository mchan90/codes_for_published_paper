...
def Apply_TrotterEvolution(isec, alpha, time, eps, n_trotter, indx, v):
    ...
    if (indx==0): 
        ...
        w = v
        w = Apply_ExactEvolution_coulomb (isec, 0.0, dt/2, w)
        ...
        # second  order trotter
        w = v
        w = Apply_ExactEvolution_kinetic (isec, alpha, 0.0, dt/2, w)
        for i_trotter in range(n_trotter-1):
            w = Apply_ExactEvolution_coulomb (isec, 0.0, dt, w)
            w = Apply_ExactEvolution_kinetic (isec, alpha, 0.0, dt, w)
        w = Apply_ExactEvolution_coulomb (isec, 0.0, dt, w)
...
def NumberOfTrotterSteps(alpha, max_n_trotter):
    trotter_factor = 1 #(n_dimer + t_inters[alpha]/t_intra * n_inter)/(n_dimer + n_inter)
