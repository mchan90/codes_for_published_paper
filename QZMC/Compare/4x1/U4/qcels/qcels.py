...
value_for_time_list     = np.zeros(len(time_max_list),dtype=float)
...
#    comm.Abort(1)
    ...
    value_for_time_list[indx_time] = ground_energy_estimate_QCELS
    if (core==0):
...
# repetition test
indx_time = 3
time_max = time_max_list[indx_time]
n_level = round(np.log2(time_max/n_time))
...
for n_sample in n_samples:
    ...
    for i_iter in range(n_iter):
        for i_level in range(n_level+1):
            ...
            lambda_max=ground_energy_estimate_QCELS+np.pi/(2*tau_scale_qcels[i_level])
        # sum error
        error += np.abs(ground_energy_estimate_QCELS-value_for_time_list[indx_time])
    error /= n_iter
