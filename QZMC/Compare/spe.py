...
d_list = [13*i for i in range(1,101)]
...
for d in d_list:
    ...
    max_evolution_time_list[indx_delta] = C99*d
    indx_delta += 1
...
for n_sample in n_samples:
    ...
    for i_iter in range(n_iter):
        ...
        # cost estimation
        total_evolution_time = np.sum(samples) * tau_scale
        max_evolution_time = np.max(samples) * tau_scale
        #print(max_evolution_time,t_max)
    ...
    error_avg /= n_iter
    if (core==0):
        print(n_sample,max_evolution_time,error_avg)
...
if (core==0):
    with open('dE_vs_repetitions_with_exact','w') as file_:
        s = '# eps= '+str(eps)+', n_shot= '+str(n_shot)+', n_iter='+str(n_iter)
        s += '\n'
        file_.write(s)
        indx_n_sample = 0
        for n_sample in n_samples:
...
eps = delta/tau_scale
#delta = eps*tau_scale
n_sample = 16000 # after the convergence
#dt_trotters = [10.0, 5.0, 1.0, 0.5, 0.1, 0.05]
#max_n_trotter_list = [40, 80, 400, 800, 4000, 8000]
...
for max_n_trotter in max_n_trotter_list:
    ...
    for ii in range(core,n_alive,cores):
        ...
        alpha = n_hamiltonians-1
        time = tau_scale * k
        #n_trotter = int(time/dt_trotter)+1
        #n_trotter = max(round((k/d)**2 * max_n_trotter),1) # square 
        n_trotter = max_n_trotter
        n_trotters[k] = n_trotter
        #u_exact = ExactEvolution(isec,alpha,0.0,time)
        indx = 1
        phi = copy.copy(phi_start)
...
#derivs = CDF_deriv_from_evolution_importance2(amplitudes_sample, Coeffs_sum,samples, xs)

# %%
#maxloc = np.argmax(derivs)
#print(xs[maxloc]/tau_scale-eigen_energies_exact[0][-1,0])
...
#x_max_plot    =  x_max/tau_scale
#
#axis.set_xlim(x_min_plot,x_max_plot)
##axis.set_ylim(y_min,y_max)
## setup labels
#
##axis.text(-0.29,0.98,r'(a)',transform=axis.transAxes)
#axis.set_xlabel(r'$x$')
#axis.set_ylabel(r"$G'(x)$")
#axis.axvline(x=-np.pi/4/tau_scale,color='gray',linestyle='--')
#axis.axvline(x=-np.pi/4/tau_scale+0.003,color='red',linestyle='--')
##axis.axvline(x=-6.07,color='blue',linestyle='--')
### setup tics
## plot
#axis.plot(xs/tau_scale, derivs)
#
#
#plt.tight_layout()
#
