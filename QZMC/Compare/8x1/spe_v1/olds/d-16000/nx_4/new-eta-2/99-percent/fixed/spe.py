...
d_list = [500, 1000, 2000, 4000, 8000, 16000, 32000, 64000, 128000]
...
value_for_delta_list = np.zeros((len(delta_list)),dtype=float)
...
for d in d_list:
    ...
    if (sol_method==1):
        ...
        cdf_1 = lambda x: CDF_from_evolution(amplitudes,Coeffs,x) - (eta_min/2.0)
    ...
    value_for_delta_list[indx_delta] = eigen_energy_estimate/tau_scale
...
for n_sample in n_samples:
    ...
    for i_iter in range(n_iter):
        ...
        if (sol_method==1):
            ...
            cdf_1 = lambda x: CDF_from_evolution_importance2(amplitudes_sample,Coeffs_sum,samples,x) - (eta_min/2.0)
            ...
            eigen_energy_estimate = result.x
        error = np.abs(eigen_energy_estimate/tau_scale-value_for_delta_list[indx_d])
...
max_n_trotter_list+= [2000,3000,4000,5000,10000]
