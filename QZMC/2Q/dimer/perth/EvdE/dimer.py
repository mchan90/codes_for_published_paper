...
service = QiskitRuntimeService(channel="ibm_quantum", instance='ibm-q-research-2/korea-inst-adv-1/main')
...
eigen_energies_direct      = np.zeros((n_hamiltonians,dim),dtype=float)
...
#    circuit_real = transpile(circuit_real,backend=backend)
    ...
    for alpha in range(1,n_hamiltonians):
        ...
        match indx_status[i][alpha]:
            ...
            case 0: # new jobs
                ...
                while (problem):
                    ...
                    # 3; H measurement
                    i_obs = 1
                    nhd1 = len(hamiltonians[alpha])
                    for ihd in range(nhd1):
                        # pass for constant contribution
                        if (hamiltonian_list[alpha][ihd][0]=='I'*n_qubit):
                            continue
                        for imc in range(nmc):
                            U_evol = np.identity(dim,dtype=complex)
                            phase  = 0.0
                            ...
                            # insertion of (H[alpha])
                            Mat = hamiltonians[alpha].paulis[ihd].to_matrix()
        ...
        for ihd in range(nhd1):
            if (hamiltonian_list[alpha][ihd][0]=='I'*n_qubit):
                continue
            coeff = hamiltonian_list[alpha][ihd][1]
            for imc in range(nmc):
                E +=  coeff * result_values_save[i][alpha][i_meas]
                i_meas += 1
        ...
        print(alpha, norm, eigen_energies_direct[alpha,i]-eigen_energies_exact[alpha,i])
