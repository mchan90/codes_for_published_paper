...
with open('H2.time.binary','wb') as file_:
...
# ibmq_qasm_simulator, exact simulator, exact time evolution, with nshot
...
# for a real machine
#z_hadamard_test = SparsePauliOp.from_sparse_list([('Z',[0],1)],num_qubits=num_qubits_backend)
...
#with open('H2.job_ids','r') as file_:
...
#            ind_line += 1
#
#with open('H2.params.binary','rb') as file_:
#    [params_list_save] = pickle.load(file_)
#
#with open('H2.indx.status','rb') as file_:
    ...
    for alpha in range(1,n_hamiltonians):
        ...
        match indx_status[i][alpha]:
            ...
            case 1: # ongoing job
                ...
                circuits = [circuit_real.bind_parameters({params: params_}) for params_ in params_list]
                with Session(service=service, backend=backend_name) as session:
            ...
            case 0: # new jobs
                ...
                with Session(service=service, backend=backend_name) as session:
                    ...
                    with open('H2.job_ids','w') as file_:
                        for i_ in range(dim):
                            for alpha_ in range(1, n_hamiltonians):
                                ...
                                s += '\n'
                                file_.write(s)
                    with open('H2.params.binary','wb') as file_:
                        pickle.dump([params_list_save],file_)
                    with open('H2.indx.status','wb') as file_:
...
with open('H2.result.values.binary','wb') as file_:
