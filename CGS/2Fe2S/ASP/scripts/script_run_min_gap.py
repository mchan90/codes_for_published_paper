...
for j in range(len(indx_list)):
    ...
    folder_sub = folder + '/gap_min_finding'
    ...
    with open('./find_min_gap.sh','r') as file_:
        lines = []
        lines_slurm = file_.readlines()
        for line in lines_slurm:
            if (line.startswith('#SBATCH --job-name')):
                st = '#SBATCH --job-name={indx}_gap'.format(indx=indx) +'\n'
                lines.append(st)
                ...
                lines.append(line)
    fname = folder_sub + '/find_min_gap.sh'
    with open(fname,'w') as file_:
        file_.writelines(lines)

    with open('./find_min_gap.py','r') as file_:
        lines = []
        lines_slurm = file_.readlines()
        for line in lines_slurm:
            if (line.startswith('eps_x')):
                ...
                lines.append(line)
    fname = folder_sub + '/find_min_gap.py'
    with open(fname,'w') as file_:
        file_.writelines(lines)
    
    fname = 'find_min_gap.sh'
    cmd = 'sbatch ' + fname
