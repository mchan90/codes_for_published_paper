...
#def brackets_from_grid_all_nomerge(x, g, fallback=True):
#    """
#    x: strictly increasing 1D array (len >= 3)
#    f: same length
#    반환: [(iL, iB, iR), ...]  # 모든 로컬 최소(center)별 브래킷, 왼→오
#    - plateau(연속 center)도 각 인덱스를 모두 개별 브래킷으로 반환
#    - fallback=True면 내부 로컬 최소가 하나도 없을 때 단일 브래킷을 하나 반환
#    """
#
#    x = np.asarray(x)
#    g = np.asarray(g)
...
#
#    triples = [(i-1, i, i+1) for i in centers]  # 자연스럽게 오름차순
#
...
def bracket_from_grid_simple(x, g):
    """
    x: strictly increasing 1D array (len >= 3)
    f: same length
...
taus = []
s_taus = []
with open('../qz/optimal_schedule','r') as file_:
    lines = file_.readlines()
    for line in lines:
        ls = line.split()
        taus.append(float(ls[0]))
        s_taus.append(float(ls[1]))

from scipy.interpolate import PchipInterpolator
from scipy.optimize import brentq
pchip = PchipInterpolator(taus, s_taus)
...
with open('../est/T_asp_est','r') as file_:
    ...
    for line in lines:
        ...
        if (s>=1): # 1
            ...
            func = lambda x: pchip(x) - s
            tau = brentq(func, 0, 1)
        deriv_value = pchip_deriv(tau)
        T_w_opt = T_bare * deriv_value
        s_list.append(s)
        T_list.append(T_w_opt)
...
if (core==0):
    ...
    with open('T_asp_est','w') as file_:
        for i in range(slen):
            s = '{:.16e}    {:.16e}    {:.16e}'.format(s_list[i], T_list[i], gap_list[i])
            s += '\n'
            file_.write(s)


dip_list = []
dip = bracket_from_grid_simple(s_list, T_list)
dip_list.append(dip)
...
for iter_ in range(iter_max):
    ...
    for s_core in s_cores:
        ...
        T_asp_est_bare, gap = compute_T_est_and_gap(ci, e+ecore)
        
        # multiply derivatie
        if (s_core>=1): # 1
            tau = 1.0
        else:
            func = lambda x: pchip(x) - s_core
            tau = brentq(func, 0, 1)
        deriv_value = pchip_deriv(tau)
        T_asp_est = T_asp_est_bare * deriv_value
    ...
    i_min, i_mid, i_max = bracket_from_grid_simple(ordered_s_filtered, ordered_T_filtered)
    ...
    #DeltaS<eps_x or 
    if (is_last):
        j = int(np.argmax(ordered_T))  # 0=왼쪽, 1=가운데, 2=오른쪽 최소
        print ('# T maximum is found at ', ordered_s[j])
        print ('# with the T maximum value of ', ordered_T[j])
