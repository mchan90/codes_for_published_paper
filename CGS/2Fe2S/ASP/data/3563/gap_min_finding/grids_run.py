...
iter_max = int(np.log(DeltaS/eps_x)/np.log((n_add+1)/2)) + 1

print("cores: ",cores, "eps_x: ", eps_x)
...
for iter_ in range(iter_max):
    if (cores==0):
        ...
        s_core = ss[0] + (core+1)/(cores+1) * DeltaS
        s_cores = [s_core]
    ...
    all_results.append([(ss[0], Ts[0], gaps[0])])
    all_results.append([(ss[2], Ts[2], gaps[2])])
    ...
    # check convergence
    i_min, i_mid, i_max = bracket_from_grid_simple(ordered_s, ordered_g)

    ss = ordered_s[i_min:i_max+1]
    print(ss)
    Ts = ordered_T[i_min:i_max+1]
    print(Ts)
    gaps = ordered_g[i_min:i_max+1]
    print(gaps)
