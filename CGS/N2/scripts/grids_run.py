...
def compute_T_est_and_gap (fcivec, e):
    ...
    max_T_ASPest = 0.0
    gap_list = []
    assert len(e) == len(fcivec) and len(fcivec) == n_roots
...
interpol_list = np.linspace(0,10000,num=101,dtype=int)
...
for interpol in my_tasks:
    ...
    e, ci = mci.kernel(h1e, g2e, norb, nelec, nroots=n_roots, max_space=30, max_cycle=2000)
    e = np.array(e)
